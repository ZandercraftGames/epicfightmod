...
public class HealthBarIndicator extends EntityIndicator
{
	...
	{
		if(!entityIn.isNonBoss() || entityIn.isInvisible() || entityIn == Minecraft.getInstance().player.getRidingEntity()) {
			return false;
	...
	{
		Matrix4f mvMatrix = super.getMVMatrix(matStackIn, entityIn, 0.0F, entityIn.getHeight() + 0.25F, 0.0F, true, false);
	...
	{
		if(entitydataFighter.getStunResistance() == 0) {
			return;
		}
